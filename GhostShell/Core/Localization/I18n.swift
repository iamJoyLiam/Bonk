...
final class I18n: ObservableObject, @unchecked Sendable {
  ...
  init() {
    ...
    NotificationCenter.default.addObserver(
      ...
      self.lock.lock()
      let savedChoice = self._savedChoice
      self.lock.unlock()
      if savedChoice == "system" {
        let newLang = Self.resolve("system", availableKeys: Array(self._allStrings.keys))
  ...
  private(set) lazy var availableLanguages: [String] = {
    Array(_allStrings.keys).sorted()
  }()
