    ...
    extension OpenSSHBackend {
        ...
        func commonOpenSSHArguments(additionalOptions: [String], attemptID: String) -> [String] {
            ...
            args += [
                ...
                "-o", "ConnectTimeout=10",
                // No Kerberos/GSSAPI: the app supplies its own credentials via
                // askpass/responder. GSSAPI otherwise adds KDC/reverse-DNS stalls
                // to every cold connect before falling back.
        ...
        func authenticationArguments(for authMethod: SSHAuthMethod?) -> [String] {
            switch authMethod {
            case .password:
                ["-o", "PreferredAuthentications=password,keyboard-interactive", "-o", "PasswordAuthentication=yes", "-o", "KbdInteractiveAuthentication=yes", "-o", "PubkeyAuthentication=no"]
            case .privateKey, .certificate:
                ["-o", "PreferredAuthentications=publickey,keyboard-interactive", "-o", "PasswordAuthentication=no", "-o", "KbdInteractiveAuthentication=yes"]
            case .secureEnclaveKey:
                ["-o", "PreferredAuthentications=publickey,keyboard-interactive", "-o", "PasswordAuthentication=no", "-o", "KbdInteractiveAuthentication=yes"]
            case nil:
                ["-o", "PreferredAuthentications=password,keyboard-interactive,publickey", "-o", "PasswordAuthentication=yes", "-o", "KbdInteractiveAuthentication=yes"]
            }
        }
