...
extension ByteBuffer {
    ...
    mutating func writeSSHHostKey(_ key: NIOSSHPublicKey) -> Int {
        ...
        case .custom(let key):
            // RFC 8332: rsa-sha2-256/512 host keys stay "ssh-rsa" on the wire,
            // and the exchange-hash transcript must use the wire identifier.
            if let rsaKey = key as? RSASHA2PublicKeyBase {
                writtenBytes += writeSSHString("ssh-rsa".utf8)
                writtenBytes += rsaKey.write(to: &self)
            } else {
    ...
    mutating func readPublicKeyWithoutPrefixForIdentifier<Bytes: Collection>(_ keyIdentifierBytes: Bytes) throws -> NIOSSHPublicKey? where Bytes.Element == UInt8 {
        try self.rewindOnNilOrError { buffer in
            if keyIdentifierBytes.elementsEqual(NIOSSHPublicKey.ed25519PublicKeyPrefix) {
                ...
                // RFC 8332: rsa-sha2-256 / rsa-sha2-512 host keys keep the
                // legacy "ssh-rsa" identifier in the key blob. Parse them with
                // an rsa-sha2 public key type so signature verification can
                // pick the negotiated digest.
                if keyIdentifierBytes.elementsEqual("ssh-rsa".utf8) {
                    for type in NIOSSHPublicKey.customPublicKeyAlgorithms where type.publicKeyPrefix.hasPrefix("rsa-sha2-") {
